...
public void OnPluginStart()  
{
	...
	Handle GetStatsString = EndPrepSDKCall();
	if(!GetStatsString) 
	...
	Handle hGetStatsString = LoadGameConfigFile("sv-var.games");
	PrintToServer("var: ~tick %6.2f", view_as<float>(LoadFromAddress(GameConfGetAddress(hGetStatsString, "host_framestarttime_stddeviation"), NumberType_Int32)) * 1000);
	PrintToServer("+- %4.3f ms", view_as<float>(LoadFromAddress(GameConfGetAddress(hGetStatsString, "host_frametime_stddeviation"), NumberType_Int32)) * 1000);
	PrintToServer("sv: Svms %5.3f", view_as<float>(LoadFromAddress(GameConfGetAddress(hGetStatsString, "host_frameendtime_computationduration"), NumberType_Int32)) * 1000);
